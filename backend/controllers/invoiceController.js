...
const addInvoice=asyncHandler(async(req,res)=>{
	var invoiceObj = new Invoice({ user_id:req.body.user_id, company_id:req.body.company_id, client_id:req.body.client_id, invoiceNo: req.body.formData.invoiceNo, status: req.body.formData.status?req.body.formData.status:'Draft' ,invoiceDate: req.body.formData.invoiceDate ,dueDate: req.body.formData.dueDate ,paidDate: req.body.formData.paidDate });
	invoiceObj.save(function (err, invoice) {
		if (err) return res.status(400).json({ err: err })
		if (invoice) {
			req.body.formData.products.map((products, index) => {
				var productsObj = new Product({
					...
					total: (products.price*products.qty)-products.discount,
				});
				productsObj.save((error, savedTask) => {
					if (err) {
						...
						productsObj.deleteOne({ invoice_id: invoice._id }, function (err) {
							return res.status(400).send({ err: error });
						});
						return res.status(400).send({ err: error });
					}
...
const updateInvoice = asyncHandler(async (req, res) => {
  ...
  if (invoice) {
    ...
    req.body.formData.products.map(async(product,index)=>{
      if(product._id){
       ...
       console.log(savedProduct)
        if(savedProduct){
          ...
          savedProduct.subtotal=product.subtotal||savedProduct.price*savedProduct.qty,
          savedProduct.total=product.total||(savedProduct.price*savedProduct.qty)-savedProduct.discount;
        }
        if(savedProduct){
...
const getAllInvoicesWithProduct=asyncHandler(async(req,res)=>{
  ...
  const getInvoiceByClientIdAndInvoiceDate=asyncHandler(async(req,res)=>{
    ...
    const invoice=await Invoice.find({client_id:client_id,invoiceDate:invoiceDate})
    if(invoice){
      res.send(invoice)
    }
  })
...
module.exports = { addInvoice,updateInvoice,deleteInvoice ,getInvoiceByUser_id,getAllInvoicesWithClientInfo,getInvoiceByClientId,getInvoiceByClientIdAndInvoiceDate,getAllInvoicesWithProduct,deleteProduct,getInvoiceWithProducts};